...
class Player():
    ...
    def one_turn(self, start_row, start_column, target_row, target_column, gameboard): 
        ...
        if (s_row < 0 or s_row > 9) or (s_column < 0 or s_column > 9) or (t_row < 0 or t_row > 9) or (t_column < 0 or t_column > 9):
            display_message("VEUILLEZ SAISIR DES COORDONNEES SUR LE PLATEAU.", "red")
        ...
        #On reste dans les coordonnées du plateau.
            ...
            if int(gameboard[s_row][s_column]) == self.number:
                self.take_checker(s_row, s_column, t_row, t_column, gameboard, factor)
            ...
            #Le joueur n'a pas pris son pion.
                ...
                display_message("PS : Au fait, vos pions sont les %d ." %self.number, "black")
                return {"message": "pb"}
    ...
    def take_checker(self, s_row, s_column, t_row, t_column, gameboard):
        ...
        #Le joueur ne met pas de bonnes coordonnées.
            ...
            display_message("VOUS NE POUVEZ PAS PLACER VOTRE PION ICI. RECOMMENCEZ.","red")
    ...
    def take_king(self, s_row, s_column, t_row, t_column, gameboard):
        ...
        if abs(t_column - s_column) == factor_king and int(gameboard[t_row][t_column]) == 0:
            ...
            if len(count_capture) == 1:
                ...
                display_message("VOUS NE POUVEZ PAS PLACER VOTRE DAME ICI. RECOMMENCEZ.","red")
    ...
    def win_one_point(self):
        self.score += 1
        display_message("Le joueur %d prend un point." %(self.number), "purple")
    ...
    def play(self, gameboard):
        ...
        while True:
            try:
                ...
                display_message("Veuillez rentrer des coordonnée de cases.", "red")
                display_message("Les lignes et les colonnes commencent à 1 !", "black")
                view(gameboard)
                
        while True:
            try:
                start_column = int(input("Sur quelle colonne se situe votre pion ? "))
                break
            except ValueError:
                display_message("Veuillez rentrer des coordonnée de cases.", "red")
                display_message("Les lignes et les colonnes commencent à 1 !", "black")
        ...
        while True:
            try:
                target_row = int(input("Sur quelle ligne voulez-vous bouger votre pion ? "))
                break
            except ValueError:
                display_message("Veuillez rentrer des coordonnée de cases.", "red")
                display_message("Les lignes et les colonnes commencent à 1 !", "black")
        ...
        while True:
            try:
                target_column = int(input("Sur quelle colonne voulez-vous bouger votre pion ? "))
                break
            except ValueError:
                display_message("Veuillez rentrer des coordonnée de cases.", "red")
                display_message("Les lignes et les colonnes commencent à 1 !", "black")
